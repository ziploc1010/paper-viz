...
export default function LRMComplete() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setError('Failed to load LRM model data.');
        setLoading(false);
      }
    };
    loadData();
  ...
  useEffect(() => {
    ...
    const handleComponentClick = (e) => {
      ...
      setActiveTab(null); // Reset active tab when selecting new component
    };
  ...
  const getComponentData = () => {
    ...
    // Set default tab based on component type
    if (!activeTab && quiz) {
      if (quiz.concepts) {
        setActiveTab('concepts');
      } else if (quiz.equations) {
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4 flex-shrink-0">
        <h1 className="text-2xl font-bold">📊 LRM Efficiency Hierarchy - Interactive Quiz</h1>
        <p className="text-gray-600 text-sm mt-1">Click on any component or paper group in the diagram to explore its details</p>
      </div>
        ...
        >
          ...
          <>
            ...
            <div className="flex border-b border-gray-200 mb-6">
              {componentData.concepts ? (
                // Leaf node with concepts only
                <button
                  className="px-4 py-2 font-medium text-sm border-b-2 transition-colors text-blue-600 border-blue-600"
                >
                  📋 Concepts
                </button>
              ) : (
                // Non-leaf node with equations, initialization, and forward pass
                <>
                  <button
            ...
            <div className="tab-content">
              {/* Concepts tab for leaf nodes */}
              {activeTab === 'concepts' && componentData.concepts && (
                <div className="space-y-6">
                  {componentData.concepts.map((concept, index) => {
                    const conceptKey = `${selectedComponent}_concept_${index}`;
                    return (
                      <div key={index} className="bg-gray-50 rounded-lg p-6">
                        ...
                        <p className="text-gray-700 mb-4">
                          <strong>Question:</strong> {concept.question}
                        </p>
                        <div className="relative">
                          <button 
                            onClick={() => toggleVisibility(conceptKey)}
                          ...
                          >
                            {visibility[conceptKey] === false ? 'Show' : 'Hide'}
                          </button>
                          {visibility[conceptKey] !== false ? (
                            <div className="bg-white p-4 rounded border border-gray-200">
                              <p className="text-gray-800">{concept.answer}</p>
                            </div>
                          ) : (
              ...
              {/* Original tabs for non-leaf nodes */}
              {activeTab === 'equations' && !componentData.concepts && (
                <div className="space-y-6">
